...
export default function CommunityPost({ post }: { post: Post }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setComments([d.comment, ...comments]); setName(''); setBody(''); setStatus('Comment added.'); setLoaded(true);
  }
  ...
  return <article className="communityPost card">
    <div className="cardbody">
      ...
      <div className="comments">
        <div className="commentSectionHeader"><strong>{loaded ? `${comments.length} comment${comments.length===1?'':'s'}` : 'Join the discussion'}</strong><button className="btn" onClick={loadComments}>{loaded ? 'Refresh comments' : 'View comments'}</button></div>
        ...
        {loaded && <div className="commentList">{comments.map(c=><div className={`comment ${c.is_admin ? 'adminComment' : ''}`} key={c.id}><div className="commentMeta"><strong>{c.display_name}</strong>{c.is_admin && <span className="adminBadge">ADMIN</span>}<span className="small">{new Date(c.created_at).toLocaleString('en-IN')}</span></div><p>{c.body}</p></div>)}</div>}
      </div>
    </div>
  </article>;
}
