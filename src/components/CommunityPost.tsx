...
export default function CommunityPost({
  ...
  const [visibleImageControls, setVisibleImageControls] = useState<number | null>(
    null
  );
  ...
  async function loadComments() {
    ...
    const r = await fetch(`/api/comments?postId=${post.id}`, {
      cache: 'no-store',
    });
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!r.ok) {
      setStatus(d.error || 'Could not post comment.');
      return;
    }
  ...
  return (
    <>
              ...
              {post.image_urls.map((url, i) => (
                <div
                  ...
                  className={`communityImageWrap ${
                    visibleImageControls === i ? 'showControls' : ''
                  }`}
                  onMouseEnter={() => setVisibleImageControls(i)}
                  onMouseLeave={() => setVisibleImageControls(null)}
                  onClick={() => setVisibleImageControls(i)}
                  role="group"
                  aria-label={`Image ${i + 1} of ${post.title}`}
                >
                  <img
                    src={url}
          ...
          <div className="comments">
            <div className="commentSectionHeader">
              ...
              <button className="btn" onClick={loadComments}>
                {loaded ? 'Refresh comments' : 'View comments'}
              </button>
            </div>
      ...
      <style jsx>{`
        ...
        .communityImageWrap:hover .communityImageFullView,
        .communityImageWrap.showControls .communityImageFullView {
          opacity: 1;
        ...
        .communityImageLightbox {
          ...
          animation: communityFadeIn 0.16s ease;
        }
