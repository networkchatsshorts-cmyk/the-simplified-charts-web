...
export async function GET(req: Request) {
  ...
  const { data, error } = await db.from('community_comments').select('id,display_name,body,created_at').eq('post_id', postId).eq('published', true).order('created_at', { ascending:false });
...
export async function POST(req: Request) {
  ...
  const { data, error } = await db.from('community_comments').insert({ post_id:postId, display_name:name, body:text, published:true }).select('id,display_name,body,created_at').single();
  if (error) return NextResponse.json({ error:error.message }, { status:500 });
