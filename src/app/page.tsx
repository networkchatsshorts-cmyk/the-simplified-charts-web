...
export default async function HomePage() {
  ...
  const [{ data: topics }, { data: videos }] = await Promise.all([
    ...
    db
      .from('videos')
      .select(
        'id,title,slug,thumbnail_url,published_at,topic_id,seo_description'
      )
      .eq('published', true)
  ...
  /*
   * Each playlist contributes only its latest published video.
   * Then those latest videos are globally sorted newest -> oldest.
   */
  ...
  for (const video of videos || []) {
    ...
    const current = latestByTopic.get(video.topic_id);
  ...
  const recentPlaylistVideos = (topics || [])
    .map((topic) => {
  ...
  /*
   * Shorts are kept as a separate section.
   */
  ...
  if (shortsTopic) {
    const { data } = await db
      .from('videos')
      .select(
        'id,title,slug,thumbnail_url,published_at,seo_description'
      )
  ...
  return (
    <main className="container">
      ...
      <section className="section">
        ...
        <div className="grid">
          {recentPlaylistVideos.map(({ topic, video }) => (
                ...
                <div className="cardbody">
                  ...
                  <p className="small">
                    {video.seo_description ||
                      'Latest analysis from this playlist.'}
                  </p>
        ...
        {!recentPlaylistVideos.length && (
          <div className="card">
            <div className="cardbody">
              <p className="small">
                No playlist videos are available yet.
              </p>
            </div>
          </div>
        )}
